...
if (_host !== undefined) {
    ...
    var _require_dictionary = function (_callback) {
        try {
            ...
            // console.log('sj ', finalseg)
        }
        catch (e) {
            console.log('sj err', e)
    ...
    _require_dictionary(() => {
        require([ _host +'scripts/tfidf.js' ], function (_tfidf) {
            console.log('sj ??', _tfidf)
            _tf = _tfidf

            doAnalyze = (text) => {

                return new Promise((res, rej) => {
                    call_jieba_cut(text, PREDIFINED_DICTIONARY, (r) => {
                        res(_tfidf.analysis(r.join(" ")))
                    })
                })
